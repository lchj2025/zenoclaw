...
async function startServer() {
  ...
  app.get('/api/platforms', async (req, res) => {
    try {
      const { listPlatforms, getPlatformMeta } = await import('../platforms/loader.js')
      const platforms = listPlatforms()
      const fs = await import('fs')
      const path = await import('path')
      const { fileURLToPath } = await import('url')
      const platformsDir = path.dirname(fileURLToPath(import.meta.url)).replace(/api$/, 'platforms')
      ...
      const result = await Promise.all(platforms.map(async (name) => {
        ...
        const meta = await getPlatformMeta(name)
        // interact 能力判断：有专用文件 或 适配器提供了 interactSelectors
